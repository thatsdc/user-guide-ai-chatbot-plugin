...
async def test_jenkins_docs(tmp_path: Path, ephemeral_vector_db: QdrantVectorStore):
    SOURCE = DataSource.JENKINS_DOCS
    ...
    output_dir.mkdir()
    raw_dir.mkdir()
    write_json_file(raw_dir / f"{SOURCE.value}.json", COLLECTOR_OUTPUT_JENKINS_DOCS)
